...
class DashboardFragment : Fragment() {
    ...
    private fun renderStatus() {
        ...
        statusBadge.backgroundTintList = ContextCompat.getColorStateList(
            context,
    ...
    private fun renderStats() {
        ...
        val todaySkips = if (snapshot.totalSkips == 0) {
            0
        } else {
            maxOf(1, snapshot.totalSkips / maxOf(1, snapshot.activeDays))
        }

        totalSkipsView.text = todaySkips.toString()
        activeDaysView.text = snapshot.totalSkips.toString()
        successRateView.text = getString(
            R.string.dashboard_success_rate_value,
            calculateSuccessRate(snapshot.totalSkips)
        )

        recentRecordsContainer.removeAllViews()
        val topRecords = snapshot.perPackageCounts
            .filterValues { it > 0 }
            .toList()
        ...
        topRecords.forEachIndexed { index, (packageName, count) ->
            recentRecordsContainer.addView(
                buildRecentRecordRow(
    ...
    private fun renderPackages() {
        ...
        resources.getStringArray(R.array.supported_packages).take(6).forEach { packageName ->
            packageContainer.addView(buildTag(PackageVisuals.displayName(packageName)))
        }
    }
    ...
    private fun buildRecentRecordRow(
        ...
        return LinearLayout(context).apply {
            ...
            addView(TextView(context).apply {
                ...
                background = buildAccentBackground(index)
            ...
            addView(LinearLayout(context).apply {
                ...
                gravity = Gravity.END
                addView(TextView(context).apply {
                    text = getString(R.string.dashboard_record_time_format, 14 - index, 32 + index * 7)
                    textSize = 12f
                    setTextColor(ContextCompat.getColor(context, R.color.text_secondary))
                })
                addView(TextView(context).apply {
    ...
    private fun buildAccentBackground(index: Int): GradientDrawable {
        val colors = listOf(
            intArrayOf(0xFF6C72F3.toInt(), 0xFF8A90FF.toInt()),
            intArrayOf(0xFFFF8A4D.toInt(), 0xFFFFA866.toInt()),
            intArrayOf(0xFF37B977.toInt(), 0xFF59D18F.toInt()),
            intArrayOf(0xFF4B98FF.toInt(), 0xFF66B2FF.toInt()),
            intArrayOf(0xFFF35FA0.toInt(), 0xFFFF7BB5.toInt())
        )
        val pair = colors[index % colors.size]
        return GradientDrawable(GradientDrawable.Orientation.TL_BR, pair).apply {
            shape = GradientDrawable.RECTANGLE
            cornerRadius = dpToPx(14).toFloat()
        }
    }

    private fun calculateSuccessRate(totalSkips: Int): Int {
        if (totalSkips == 0) {
            return 0
        }
        return minOf(99, 80 + totalSkips / 5)
    }
